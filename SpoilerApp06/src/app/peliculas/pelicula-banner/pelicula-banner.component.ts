...
export class PeliculaBannerComponent implements OnInit, OnChanges {
  ...
  @Output() dioClickEnEstado: EventEmitter<boolean> = new EventEmitter<boolean>();
  textoEstreno: string;
  claseEstreno: string;
  ...
  setearColorYClase() {
    if (this.esEstreno) {
      ...
      this.claseEstreno = 'sa-fondo-estado-rosado';
    } else {
      this.textoEstreno = 'Proximamente';
      this.claseEstreno = 'sa-fondo-estado-amarillo';
    }
  }
  hizoClickEnEstado() {
